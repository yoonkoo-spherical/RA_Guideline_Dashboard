...
def ask_guideline(user_query: str):
    ...
    3. 정보 출처 구분: 제공된 DB 데이터 기반 서술은 **[DB 참조]**로, 웹 검색을 통해 보완된 데이터는 **[웹 참조]**로 표기하여 시각적으로 분리하십시오. 인용한 내용의 출처(URL, 문서명)를 답변의 마지막에 목록으로 명시하십시오.
    4. 한국어 존댓말을 사용하며, 가독성을 위해 마크다운 표와 글머리 기호를 활용하십시오.
    """
...
def compare_multiple_documents(docs_info, user_query: str = "위 문서들을 종합적이고 심층적으로 비교 분석하십시오."):
    ...
    try:
        ...
        3. 인용: 특정 내용을 서술할 때 문서명 등 식별 정보를 반드시 포함하십시오. 웹 검색 내용도 출처 url을 포함하여 문서의 마지막에 배치하십시오.
