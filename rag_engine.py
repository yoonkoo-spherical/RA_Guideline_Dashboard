...
def ask_guideline(user_query: str):
    ...
    def get_document_change_history(target_title_keyword: str) -> str:
        ...
        if not docs:
            return f"내부 DB에서 '{target_title_keyword}'에 대한 개정 이력을 찾을 수 없습니다."
    ...
    [상황별 답변 및 도구 활용 원칙]
    질문의 성격에 따라 당신의 전문 지식과 내부 검색 도구를 유연하게 결합하십시오.
    ...
    3. 정보 출처 구분: 내부 DB 데이터는 **[DB 참조]**로 표기하고 인용한 내용의 출처(문서명, URL 등)를 답변에 명시하십시오. 내부 DB에 정보가 없을 경우, 무리하게 추측하지 말고 한계를 명확히 밝히십시오.
    4. 한국어 존댓말을 사용하며, 정보의 체계적 전달을 위해 글머리 기호 및 표를 활용하십시오.
    """

    try:
        # API 정책에 따라 google_search 도구와 사용자 정의 함수를 분리
        tools = [
        ...
        def _generate_comparison():
            return client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    system_instruction=system_instruction,
                    temperature=0.0, 
                    # 사용자 정의 함수가 없으므로 google_search 도구 단독 사용 가능
