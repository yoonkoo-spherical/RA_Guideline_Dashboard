...
def ask_guideline(user_query: str):
    ...
    def search_guideline_database(content_query: str, target_agency: str = None, target_title_keyword: str = None) -> str:
        """
        데이터베이스 검색용 자기 질의(Self-Querying) 함수입니다.
        - content_query: 사용자가 묻는 실제 세부 내용 (필수)
        - target_agency: 특정 규제 기관(FDA, EMA, Health Canada 등) (선택)
        - target_title_keyword: 문서 제목, 고유명사, 식별자(PERs, Q1A 등) (선택)
        """
    ...
    2. 정보 출처 구분: 내부 DB 데이터는 **[DB 참조]**로 표기하고, 구글 웹 검색을 통해 보완된 데이터는 **[웹 참조]**로 표기하여 시각적으로 완전히 분리하십시오.
    3. 인용 자동화: DB 내용을 인용할 때, 제공된 컨텍스트의 출처 URL 및 문서명을 문장 끝에 명시하십시오.
