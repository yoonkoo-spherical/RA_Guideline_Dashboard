...
def process_unsummarized_docs():
    print("--- 문서 요약 분석 파이프라인 (DB Text Only) ---")
    
    response = supabase.table("guidelines").select("*").or_("ai_summary.is.null,ai_summary.ilike.*추출 불가*").not_.is_("raw_text", "null").execute()
    docs = response.data
    ...
    for doc in docs:
        ...
        if not text or "추출 불가" in text:
            print(" -> 유효한 원본 텍스트가 없어 분석을 생략합니다.")
            supabase.table("guidelines").update({"ai_summary": "추출 불가"}).eq("url", doc_url).execute()
            continue
        ...
        if ref_num != "N/A" and doc.get('ref_number') != ref_num:
            existing_docs = supabase.table("guidelines").select("url, raw_text").eq("ref_number", ref_num).neq("url", doc_url).execute().data
            if existing_docs:
                old_url = existing_docs[0]['url']
                old_text = existing_docs[0].get('raw_text', '')
                
                if old_text and "추출 불가" not in old_text:
